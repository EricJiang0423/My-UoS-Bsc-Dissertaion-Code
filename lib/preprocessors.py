...
class FeatureEngineer:
    ...
    def add_user_defined_feature(self, data):
        ...
        df["daily_return"] = df["daily_return"].replace([np.inf, -np.inf], 1)
        df["daily_return"] = df["daily_return"].fillna(1)
        df["daily_return"] = df["daily_return"].astype(float)
        confidence_level = 0.95
        threshold_quantile = 0.9
        model = arch_model(df["daily_return"], vol='Garch', p=1, q=1, dist='t')
